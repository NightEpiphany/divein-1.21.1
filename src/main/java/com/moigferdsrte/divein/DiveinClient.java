...
public class DiveinClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(ResourceLocation.fromNamespaceAndPath(Divein.MOD_ID, "dive"), 45, (player) -> {
            if (player instanceof LocalPlayer) {
                ModifierLayer<IAnimation> testAnimation =  new ModifierLayer<>();

                testAnimation.addModifierBefore(new SpeedModifier(0.8923f)); //This will be medium speed
                testAnimation.addModifierBefore(new MirrorModifier(true));
                return testAnimation;
            }
            return null;
        });
        PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(ResourceLocation.fromNamespaceAndPath(Divein.MOD_ID, "lava_dive"), 45, (player) -> {
            if (player instanceof LocalPlayer) {
                ModifierLayer<IAnimation> testAnimation =  new ModifierLayer<>();
